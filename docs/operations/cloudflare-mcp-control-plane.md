...
The last two values are runtime connector credentials, not Cloudflare control-plane credentials. Rotate either value after compromise; never commit them, pass them to the client, or add them to a launchd plist.
...
3. Set the Worker `API_ORIGIN` and `EDGE_ORIGIN_TOKEN` secrets through MCP.
4. Start the protected laptop origin and named connector with `scripts/cloudflare/run-origin.sh` and `scripts/cloudflare/run-tunnel.sh`.
5. Use the Cloudflare observability MCP to inspect Worker errors and confirm `https://nativas.ai/health` before demoing.

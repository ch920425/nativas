...
4. Query `nativas_kb` with direction, component type, category, audience, and issue hypotheses. Select no more than three stable records per retrieval and retain their record IDs.
...
9. Reconcile proposals yourself. Select exactly three distinct findings for a free audit, or one to six distinct findings across one or two complete pairs for a paid audit. Every paid finding must resolve to its page pair, target screenshot, component reference, evidence IDs, and KB IDs.
